...
**Current version: v1.0.0** — see [CHANGELOG.md](./CHANGELOG.md) for what changed and [/change-history](./change-history) for a full README snapshot at every release.
...
| `GET` | `/admin` | HTTP Basic | operator panel — see below |
| `POST` | `/admin/users/refill-all` | HTTP Basic | refill every user's hearts to full |
| `POST` | `/admin/users/{id}/refill` | HTTP Basic | refill one user's hearts to full |
...
- **Auth**: HTTP Basic Auth against a single operator credential (`ADMIN_USERNAME` / `ADMIN_PASSWORD_HASH` in `backend/.env`), checked with `bcrypt.CompareHashAndPassword` — completely separate from the JWT user-auth system, so no ordinary signup can ever become an admin. If either env var is unset, the `/admin` routes aren't mounted at all (fails closed).
- Generate a password hash with `make backend-admin-hash password=yourpassword`. **The hash contains `$` characters — always single-quote it in `.env`**, otherwise a shell `source` (as the Makefile's migrate targets do) will try to expand it as positional parameters and corrupt it.
...
- **Admin credential** is bcrypt-hashed at rest, HTTP Basic over a separate auth path from user JWTs, fails closed if unconfigured.
...
- **Semantic Versioning** (`MAJOR.MINOR.PATCH`). Current: **v1.0.0**.
...
- **Git tags** (`v1.0.0`, …) mark the commit each snapshot corresponds to.
