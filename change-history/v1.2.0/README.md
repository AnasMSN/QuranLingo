...
**Current version: v1.2.0** — see [CHANGELOG.md](./CHANGELOG.md) for what changed and [/change-history](./change-history) for a full README snapshot at every release.
...
- **Add one question** via a form (prompt, Arabic text with harakat, 2-4 options, pick the correct one), or **bulk-import a CSV** (`prompt,arabic_text,option_1..option_4,correct_option` — UTF-8, so Arabic pastes in as-is; `correct_option` is the 1-based column number of the right answer). Bad rows are skipped and reported individually rather than failing the whole import.
...
- The page renders each draft as a **preview styled like the actual Android lesson screen** (same layout/colors as `mobile/src/screens/LessonScreen.tsx`), with the correct answer highlighted for the operator's eyes only, so they can proof the question exactly as a learner would see it.
- An operator can **edit** any draft inline (fixes a typo, swaps the correct answer) before publishing, or **delete** it outright. Clicking **Approve** flips it to `approved`, making it live immediately — no redeploy, no reseed.
...
- **Semantic Versioning** (`MAJOR.MINOR.PATCH`). Current: **v1.2.0**.
...
- **Git tags** (`v1.0.0`, `v1.1.0`, `v1.2.0`, …) mark the commit each snapshot corresponds to.
